...
<1> Define https://docs.micronaut.io/snapshot/api/io/micronaut/validation/Validated.html[Validated] annotation at the class level to signal any class that requires validation.
<2> The class is defined as a controller with the http://docs.micronaut.io/snapshot/api/io/micronaut/http/annotation/Controller.html[@Controller] annotation mapped to the path `/books`
